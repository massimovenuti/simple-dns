...
struct timeval
new_timeval (time_t sec, time_t usec)
{
...
struct timeval
new_countdown (int sec, int usec)
{
    struct timeval t;
...
bool
timeout (struct timeval t, int time)
{
...
float
get_timevalue (struct timeval t)
{
    return t.tv_sec + (t.tv_usec * pow (10, -6));
}

struct timeval
op_timeval (struct timeval t1, struct timeval t2, char op)
{
    if (op == '+')
    {
...
struct timeval
op_ntimeval (struct timeval t, int n, char op)
{
    if (op == '*')
    {
