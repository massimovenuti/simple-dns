...
parse_conf (const char *file_name)
{
    ...
    for (i = 0;
	 i < TABSIZE
	 && fscanf (file, "%" STR (LIP) "[^|- ] | %" STR (LPORT) "d\n", ip,
		    &port) != EOF; i++)
    {
...
void
parse_addrs (struct res *res, char *addrs)
{
    char *token;
    char name[LNAME];
    char ip[LIP];
...
parse_res (char *res)
{
    ...
    *addrs = '\0';
    ...
    if (sscanf (res,
		" %" STR (LID) "d | %" STR (LTIME) "ld,%" STR (LTIME) "ld | %"
		STR (LNAME) "[^|- ] | %" STR (LCODE) "d | " "%" STR (LRES) "s",
