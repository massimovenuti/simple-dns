...
struct server
new_server (struct sockaddr_in6 a)
{
...
void
add_shipment (struct server *s)
{
    s->nb_shipments++;
}

void
add_reply (struct server *s, struct timeval t)
{
...
void
afprint (FILE * stream, struct sockaddr_in6 addr)
{
    char ip[LIP];
    char port[LPORT];
...
void
sfprint (FILE * stream, struct server serv)
{
...
lserv
lsrm (lserv l, struct sockaddr_in6 x)
{
    if (lsempty (l))
    {
	return lsnew ();
...
bool
lsbelong (struct sockaddr_in6 addr, lserv servs)
{
...
lserv
lssearch (lserv l, struct sockaddr_in6 x)
{
    if (lsempty (l))
    {
	return lsnew ();
