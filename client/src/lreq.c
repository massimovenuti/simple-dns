...
struct req *
new_req (lreq * l, int id, char *name, struct tab_addrs addrs)
{
    struct req req;
...
void
update_req (lreq * l, struct req *req, int id, struct tab_addrs addrs)
{
    req->id = id;
    req->dest_addrs = addrs;
    req->index = get_index (*l, *req);
...
int
get_index (lreq l, struct req req)
{
    int index = 0;
    lreq tmp;
...
bool
addrcmp (struct sockaddr_in6 a1, struct sockaddr_in6 a2)
{
    char ip1[LIP];
    char ip2[LIP];
...
bool
belong (struct sockaddr_in6 addr, struct tab_addrs addrs)
{
    bool found;
