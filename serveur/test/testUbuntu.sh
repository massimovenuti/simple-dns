...
function test_bad_arg() {
    ...
    $1 |& grep "Usage:" > /dev/null || fail "Nombre d'arguments invalide (0)"
    $1 $2 riri loulou |& grep "Usage:" > /dev/null || fail "Nombre d'arguments invalide (3)"
    fin_test
}
...
function test_req() {
    ...
    test $? -eq 0 -a $FAIL -eq 0 || fail "envoi d'un requête IPV4"
