...
struct name
new_name (char *name, struct tab_servers servs)
{
    struct name res;
...
void
free_tab_servers (struct tab_servers *s)
{
    free (s->servs);
}

void
free_tab_names (struct tab_names *n)
{
...
void
add_name (struct tab_names *tn, struct name n)
{
    if (tn == NULL)
    {
	return;
...
void
add_server (struct tab_servers *ts, struct server s)
{
    if (ts == NULL)
    {
	return;
...
int
search_name (struct tab_names n, char *name)
{
...
void *
inctab (void *dest, int len, int *max_len, size_t size_elem, int coef)
{
    if (len >= *max_len)
    {
	if (*max_len == 0)
...
char *
incstr (char *dest, size_t len, size_t *max_len, int coef)
{
    if (len >= *max_len)
    {
	if (*max_len == 0)
...
struct tab_names
parse_conf (const char *file_name)
{
...
char *
strcat_res (char *dest, size_t *len_dest, size_t *max_len_dest, char *code,
...
char *
make_res (char *dest, struct req req, struct tab_names n, size_t *len_dest,
	  size_t len_src, size_t *max_len_dest)
{
	...
	*max_len_dest - 1)
    ...
    {
	dest =
	    strcat_res (dest, len_dest, max_len_dest, SUCCESS, NULL, NULL,
			NULL);
    ...
    {
	dest =
	    strcat_res (dest, len_dest, max_len_dest, FAIL, NULL, NULL, NULL);
    }
